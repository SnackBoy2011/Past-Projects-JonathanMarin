...
in  vec4 vVertex;//add elapsed time to vVertex to move it
in  vec4 vNormal;
in  vec2 vTexture;
...
void main() {
	...
	noise = (frequency+40) * -0.02f * turbulence(0.5f * vertNormal + time);

	float b = 1.0f * cnoise(0.05f * vVertex.xyz);
	float displacement = abs(noise + b);

	noiseVec = normalize(vNormal.xyz) * displacement;
